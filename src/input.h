...
void InitializeInput() {
	...
	if (SDL_NumJoysticks() < 1) { cout << cyan << "[SDL] No controllers are plugged in." << endl; return; }
	...
	for (int i = 0; i < 4; i++) {
		...
		if (gGameController[i] == NULL) { cout << cyan << "[SDL] Controller " << (controller + i) << " error : " << SDL_GetError() << endl; }
		else { cout << cyan << "[SDL] Controller " << (controller + i) << " is plugged in." << endl; }
	}
	//Load haptic
	if (haptic >= 0) {
		haptic_device = SDL_HapticOpen(haptic);
		if (haptic_device == NULL) { cout << cyan << "[SDL] Haptic " << haptic << " error : " << SDL_GetError() << endl; } else {
...
//Fetch controller input (this is very bad)
void FetchControllerInput(int ControllerPicked = 0) {
	if ((ControllerPicked >= 4 || ControllerPicked < 0) || !gGameController[ControllerPicked]) {
		for (uint_fast8_t i = 0; i < 10; i++) { BUTTONS_GAMEPAD[i] = false; }
		return;
	}
...
//Vibrate controller
void VibrateController(double intensity, int time) {
	if (haptic_device != NULL) { SDL_HapticRumblePlay(haptic_device, float(min(1.0, intensity)), time); }
}
...
void CheckInput() {
	...
	if (use_mouse || !gGameController[0]) {
		...
		mouse_x = int(double(mouse_x) / scale); mouse_y = int(double(mouse_y) / scale);
		mouse_x = min(INTERNAL_RESOLUTION_X, max(0, mouse_x)); mouse_y = min(INTERNAL_RESOLUTION_Y, max(0, mouse_y));
		uint_fast32_t m_state = SDL_GetMouseState(NULL, NULL);
		mouse_down_l = m_state & SDL_BUTTON(SDL_BUTTON_LEFT); mouse_down_r = m_state & SDL_BUTTON(SDL_BUTTON_RIGHT);
	}
	else {
		...
		mouse_down_l = SDL_GameControllerGetAxis(gGameController[0], SDL_CONTROLLER_AXIS_TRIGGERRIGHT) > 24576;
